...
public class KeyWordString {
	...
	static int SetWordArray(int nType, String key, String SecKey,
			String judgeKey, float sc1, float sc2, String sendCMD,
			int nHandleFuncIdx, int nBroadCastFlag) {
		int nIdx = 0;
		if (nType == 0) {
			nIdx = mSpcWordEndIdx;
			mSpcWordEndIdx++;
	...
	public static int initWordArray() {
		...
		SetWordArray(1, "开始随动", "", "您想人脸随动吗", 50, 30, "28", 0, 0);
	...
	private static void setYes(Context context) {
		if (mLastIdx >= 0) {
			sendBroadCast(context, mWordArray[mLastIdx].sendCMD,
					mWordArray[mLastIdx].nBroadCastFlag);
		}
	}
	...
	public static boolean handleWord(Context context, ResultBin fromJson,
			float sc, int nIdx) {
		if (sc > mWordArray[nIdx].sc1) {
			...
			if (mWordArray[nIdx].nHandleFuncIdx == 101) {
				...
				sendBroadCast(context, mWordArray[nIdx].sendCMD,
						mWordArray[nIdx].nBroadCastFlag);
			}
	...
	public static boolean handleKeyWord(Context context, ResultBin fromJson,
			float sc) {
		int i = 0;
		if (fromJson != null) {
			if (fromJson.ws.size() == 1) {
				...
				for (i = mMulWordStartIdx; i < mMulWordEndIdx; i++) {
					if (result1.equals(mWordArray[i].key)
							&& result2.equals(mWordArray[i].SecKey)) {
						handleWord(context, fromJson, sc, i);
						return true;
					}
				}
